...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{Images/dh.png}
...
\subsubsection{``Harvest now, decrypt later.''} \label{hndl}
A common misconception about the quantum threat is that it only becomes relevant when sufficiently large quantum computers become available, but this view overlooks an already active risk model known as the ``harvest now, decrypt later'' (HNDL) \cite{hndl}.
...
In the future, when PQC algorithms have withstood enough cryptanalytic scrutiny to justify sole reliance on them, it may become viable to replace the hybrid construction with a purely PQC-based key exchange. PQFS would then be achieved entirely through ephemeral KEM keypairs, with no classical component required. The security guarantees would then hold against both classical and quantum adversaries without the overhead of maintaining two parallel key exchanges.
...
Another challenge is the integration of PQC algorithms into existing protocols, as these were originally designed for classical key-exchange primitives and do not always directly support the encapsulate/decapsulate model of PQC schemes. As a result, careful modifications are required to ensure correct functionality and maintain security guarantees. In the case of TLS 1.3, for instance, changes are needed to accommodate larger key shares and to support hybrid key exchange mechanisms.

Finally, PQC implementations must be designed with strong resistance to side-channel attacks. 
Like classical cryptographic algorithms, PQC schemes can leak sensitive information through 
timing differences, power consumption, or electromagnetic emissions. Mitigating timing-based 
leakage in particular requires constant-time implementations, where the execution time of 
cryptographic operations does not vary with secret data. Recent research has demonstrated 
practical chosen-ciphertext side-channel attacks against HQC implementations 
\cite{Goy2022HQCSideChannel}, where an adversary with physical access submits carefully 
crafted ciphertexts and observes electromagnetic emissions during decapsulation to fully 
reconstruct the secret key. The attack targets the Reed-Muller decoding step of HQC's 
decapsulation, and the authors propose a masking-based countermeasure as a mitigation. 
This highlights the importance of careful engineering practices in any real-world PQC deployment.