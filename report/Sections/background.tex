...
\begin{figure}[H]
\centering
\includegraphics[alt={Diagram illustrating the Diffie-Hellman key exchange between two parties, Alice and Bob. Step 1: Alice and Bob agree on public parameters, p and g, which are large numbers. Step 2: Alice generates a private key a and computes her public value A = g^a mod p; Bob generates a private key b and computes his public value B = g^b mod p. Step 3: Alice and Bob exchange their public values A and B over an insecure channel. Step 4: Alice computes the shared secret as B^a mod p; Bob computes it as A^b mod p, both arriving at the same shared secret key g^(ab) mod p.}, width=0.8\textwidth]{Images/dh.png}
...
\begin{figure}[H]
\centering
\includegraphics[alt={Diagram illustrating key establishment between Alice and Bob using a Key Encapsulation Mechanism (KEM). Step 1: Alice generates a key pair consisting of a public encapsulation key and a private decapsulation key, and sends the encapsulation key to Bob. Step 2: Bob uses Alice's public key to run the encapsulation algorithm, producing a shared secret and a ciphertext. Step 3: Bob sends the ciphertext to Alice over the channel. Step 4: Alice uses her private key and the received ciphertext to run the decapsulation algorithm, recovering the same shared secret. Both parties now share a shared secret without it having been transmitted directly.}, width=0.8\textwidth]{Images/kem.png}
...
It is important to understand what part of modern encrypted communication is vulnerable in this context. While data at rest is protected by symmetric encryption algorithms such as Advanced Encryption Standard (AES), which are considered resistant to quantum attacks, the asymmetric public-key algorithms used during the key exchange are the primary target. If an adversary can intercept the key exchange step of a session protected by TLS for example, they can store it and later use a quantum computer to recover the symmetric session key and decrypt all the session data. The implication is that even data which appears to be well-protected at rest may be vulnerable if the key exchange that established its protection was recorded.
...
\subsection{Code-Based Cryptography (Hamming Quasi-Cyclic)}\label{sec:hqc}
...
Because the code is public rather than hidden, HQC needs a larger code length than a scheme that relies on concealed structure would, in order to keep decoding hard for an attacker. This is the underlying reason HQC's public keys and ciphertexts are significantly larger than those of ML-KEM.
